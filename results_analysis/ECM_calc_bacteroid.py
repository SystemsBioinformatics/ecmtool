...
medium2 = [ex_reac for ex_reac in ex_reac_tuples if ex_reac[1] < 0.]
ex_reac_tuples3 = [(re_id, mod.getReaction(re_id).getValue()) for re_id in mod.getReactionIds() if re_id[:4] == 'R_SK']
medium3 = [ex_reac for ex_reac in ex_reac_tuples if ex_reac[1] < 0.]

fva_result = cbm.CBCPLEX.cplx_FluxVariabilityAnalysis(mod, selected_reactions=ex_reacs)
nzrc_ex_reacs = [(re_id, fva_result[0][re_ind, 2], fva_result[0][re_ind, 2], mod.getReaction(re_id).getValue()) for
...
# for reac_id in minimal_medium:
#     if reac_id not in essential_inputs:
#         reac = mod.getReaction(reac_id)
#         reac_lb = mod.getFluxBoundByReactionID(reac_id, 'lower')
#         reac_lb.setValue(0.)

# From the preceding analysis it turns out that we have to add either ala_D or ala_L. Let's add ala_L
essential_inputs.append('R_EX_ala__L_e')
